...
public class Problem301 {
    ...
    public void remove(String s, List<String> ans, int start_count_pos, int start_remove_pos, char[] par) {

        for (int stack = 0, i = start_count_pos; i < s.length(); ++i) {
            if (s.charAt(i) == par[0]) stack++;
            if (s.charAt(i) == par[1]) stack--;
            if (stack >= 0) continue;       // 表明当前 ( 比 ) 多 正常

            // 表明当前 ) 比 ( 多  需要开始删除 ) 字符
            // 下面的for循环就是找到要删除的 ) 同时如果出现连续的 ) 只能删除第一个 否则会出现重复的结果
            for (int j = start_remove_pos; j <= i; ++j) {
                //两种情况
                // 1. (s.charAt(j) == par[1] &&  s.charAt(j - 1) != par[1]  检查重复 只移除梁旭重复字符的第一个位置
                // 2. s.charAt(j) == par[1] && j == start_remove_pos 若移除的位置就是开始位置 不需要检查重复

                if (s.charAt(j) == par[1] && (j == start_remove_pos || s.charAt(j - 1) != par[1])) {
                    // 递归调用start_count_pos还是i, start_remove_pos还是j, 是因为已经删除了一个字符 +1 -1 抵消
                    remove(s.substring(0, j) + s.substring(j + 1, s.length()), ans, i, j, par);
                }
            }
            // 递归调用remove以及高处理了后面的所有字符串 不需要再循环了
            return;
        }
