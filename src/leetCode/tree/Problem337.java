...
public class Problem337 {
	...
	/*******************优化解法*********************
	 * 相比自己写的方法 更清晰的表达
	 * 递归到最底 自底向上算出答案 没有对一个节点重复计算因此不需要memorization
	 */
	...
	private int[] dfs(TreeNode root) {
		...
		int[] res = new int[2];
		res[0] = l[1] + r[1] + root.val;   // 该节点被rob 只能选择left和right不被rob的值
		res[1] = Math.max(l[0], l[1]) + Math.max(r[0], r[1]);  // 该节点不被rob 选择left和right各自的最大值
		return res;
	}
	...
	private int rob(TreeNode root, boolean includeRoot) {
		...
		if (includeRoot) {
			...
			result = Math.max(Math.max(Math.max(rightT + leftT, rightT + leftF), rightF + leftT), rightF + leftF);
