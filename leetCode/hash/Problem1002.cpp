 ...
 * in all strings within the list (including duplicates).  For example, if a character occurs 3 times in
...
vector<string> commonChars(vector<string> &A) {
    ...
    if (A.empty())
        return res;
    array<int, 26> cache_a{};
    array<int, 26> cache_b{};
    for (char &it : A.at(0)) {
        cache_a[it - 'a']++;
    }

    for (auto &i : A) {
        fill(cache_b.begin(), cache_b.end(), 0);
        for (char j : i) {
            if (cache_a[j - 'a'] != 0) {
                cache_b[j - 'a']++;
            }
        ...
        for (int k = 0; k < 26; ++k) {
            cache_a[k] = min(cache_a[k], cache_b[k]);
        }
    }
    for (int l = 0; l < 26; ++l) {
